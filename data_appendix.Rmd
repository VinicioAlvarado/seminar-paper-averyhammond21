...
- **nitrous oxides:** Concentration of nitrous oxides in parts per billion.
...
- **TMP1:** Temperature taken hourly, measured in degrees celcius.
...
stargazer::stargazer(as.data.frame(oz_data))
...
stargazer::stargazer(as.data.frame(co_data))
...
stargazer::stargazer(as.data.frame(so2_data))
...
stargazer::stargazer(as.data.frame(nox_data))
...
stargazer::stargazer(as.data.frame(pm25_data))
```
...
weather_data <- lapply(str_c("camswx_",years), download_and_read_data) %>% 
  ...
  select(-WSR1hrvh, -WSR1hrvd, -WSR1hrpk, -WSR1hrav, -TMP1hrvh, -TMP1hrvd, -TMP1hrav, -TMP1hrpk) %>%
  group_by(AIRS, ST_CODE, AQCR, date) %>%
  pivot_longer(cols = contains("WSR1"), names_to = "hour", values_to = "wind speed") %>%
  pivot_longer(cols = contains("TMP1"), names_to = "hour", values_to = "temperature") %>%
  pivot_longer(cols = contains("WDR1"), names_to = "hour", values_to = "wind direction") %>%
  pivot_longer(cols = contains("WHR1"), names_to = "hour", values_to = "head wind") %>%
  mutate(hour = as.numeric(str_remove(hour,"WSR1hr"))) %>%
  mutate(hour = as.numeric(str_remove(hour,"WDR1hr"))) %>%
  mutate(hour = as.numeric(str_remove(hour,"WHR1hr"))) %>%
  mutate(hour = as.numeric(str_remove(hour,"TMP1hr")))
  

stargazer::stargazer(as.data.frame(weather_data))
```
...
stargazer::stargazer(as.data.frame(metro_data))
