...
- **PM2.5:** Concentration of particulate matter_{2.5} in micrograms per cubic meter.
...
pm25_hourly <- pm25_data %>%
  group_by(AIRS, ST_CODE, AQCR, date) %>%
  pivot_longer(cols = contains("PM251"), names_to = "hour", values_to = "PM_25") %>%
...
rose_labs <- c("North", "North-Northeast", "Northeast", "East-Northeast",
...
weather_hourly <- weather_data %>%
  ...
  mutate(after = Date >="2004/01/01") %>%
  mutate(airs = AIRS) %>%
  mutate(WDR = cut(WDR1, breaks = rose_breaks, labels = rose_labs, right = FALSE, include.lowest = TRUE))
...
stargazer(pm25_regression_results, type = "latex",
          report = 'vctp',
          omit = c("hour", "WDR"),
          intercept.bottom = F,
          header = F,
...
so2_regression_results <- lm(log_so2 ~ exposed + after + after*exposed + as.factor(hour) + TMP1 + WSR1 + WDR, 
                         data = so2_regression_data) 

stargazer(so2_regression_results, type = "latex",
          report = 'vctp',
          omit = c("hour", "WDR"),
          intercept.bottom = F,
          header = F,
